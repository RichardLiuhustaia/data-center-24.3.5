...
P_r_t=model.addVars(24,lb=0,ub=100,vtype=GRB.CONTINUOUS,name='P_r_t')
#constraints
#power balance constraint:DC's power supply comes from utility grid and PV generation
model.addConstrs(1/1000*(P_idle+0.75*P_peak)*(A_BW_t[i]+A_IW_t[i]+A_R_t[i])+1/1000*(L_BW_t[i]+L_IW_t[i])*(P_peak-P_idle)/L_rate+alpha[i]*P_r_t[i]
                 ==P_grid_t[i]+P_res_t[i] for i in range(T))
model.addConstrs(1/1000*(P_idle+0.75*P_peak)*(A_BW_t[i]+A_IW_t[i]+A_R_t[i])+1/1000*(L_BW_t[i]+L_IW_t[i])*(P_peak-P_idle)/L_rate+P_r_t[i]<=
                  1/1000*(P_idle+0.75*P_peak)*A_max+1/1000*(L_BW_t[i]+L_IW_t[i])*(P_peak-P_idle)/L_rate
                  for i in range(T))
model.addConstrs(1/1000*(P_idle+0.75*P_peak)*(A_BW_t[i]+A_IW_t[i]+A_R_t[i])+1/1000*(L_BW_t[i]+L_IW_t[i])*(P_peak-P_idle)/L_rate-P_r_t[i]>=
                  1/1000*P_idle*redundant_ratio*A_max
                  for i in range(T))
...
model.setObjective(gp.quicksum(L_BW_t[i]*C_BW+L_IW_t[i]*C_IW for i in range(T))
                   +gp.quicksum(reserve_price[i]*P_r_t[i]+electricity_price[i]*P_r_t[i]*abs(alpha[i]) for i in range(T))
